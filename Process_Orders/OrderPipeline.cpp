...
void OrderPipeline::setOutputCsvPath(const std::string& path)
{
    outputCsvPath = path;
}
...
void OrderPipeline::processOrdersFromFile(const std::string& filename) 
{
    ...
    std::ofstream csvFile(outputCsvPath, std::ios::trunc);
    if (!csvFile.is_open()) {
    ...
    while (std::getline(file, line)) {
        ...
        auto it = orderFunctions.find(orderType);
            if (it != orderFunctions.end()) {
                ...
                auto snapshot = book->getPerformanceSnapshot();
                csvFile << orderType << "," << duration.count() << "," << book->executedOrdersCount << ","  << book->AVLTreeBalanceCount << ","
                        << snapshot.orderAllocations << "," << snapshot.orderDeallocations << ","
                        << snapshot.limitAllocations << "," << snapshot.limitDeallocations << ","
                        << snapshot.stopLevelAllocations << "," << snapshot.stopLevelDeallocations << std::endl;
