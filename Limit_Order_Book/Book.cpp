...
Book::~Book()
{
    for (auto& [id, order] : orderMap) {
        delete order;
        performanceSnapshot.orderDeallocations++;
    }
    orderMap.clear();

    for (auto& [limitPrice, limit] : limitBuyMap) {
        delete limit;
        performanceSnapshot.limitDeallocations++;
    }
    limitBuyMap.clear();
    ...
    for (auto& [stopPrice, stopLevel] : stopMap) {
        delete stopLevel;
        performanceSnapshot.stopLevelDeallocations++;
    }
    stopMap.clear();
}
...
void Book::addLimitOrder(int orderId, bool buyOrSell, int shares, int limitPrice)
{
    ...
    if (shares != 0)
    {
        ...
        performanceSnapshot.orderAllocations++;
        orderMap.emplace(orderId, newOrder);

        auto& limitMap = buyOrSell ? limitBuyMap : limitSellMap;
        auto limitIt = limitMap.find(limitPrice);
        if (limitIt == limitMap.end())
        {
            addLimit(limitPrice, newOrder->getBuyOrSell());
            limitIt = limitMap.find(limitPrice);
        }
        limitIt->second->append(newOrder);
...
void Book::modifyLimitOrder(int orderId, int newShares, int newLimit)
{
    ...
    if (order != nullptr)
    {
        ...
        auto limitIt = limitMap.find(newLimit);
        if (limitIt == limitMap.end())
        {
            addLimit(newLimit, order->getBuyOrSell());
            limitIt = limitMap.find(newLimit);
        }
        limitIt->second->append(order);
    }
    recordOperationStats();
}
...
std::string Book::performanceSummary() const
{
    std::ostringstream oss;
    oss << "executed_orders=" << performanceSnapshot.executedOrders
        << ",avl_balances=" << performanceSnapshot.avlTreeBalances
        << ",order_allocations=" << performanceSnapshot.orderAllocations
        << ",order_deallocations=" << performanceSnapshot.orderDeallocations
        << ",limit_allocations=" << performanceSnapshot.limitAllocations
        << ",limit_deallocations=" << performanceSnapshot.limitDeallocations
        << ",stop_allocations=" << performanceSnapshot.stopLevelAllocations
        << ",stop_deallocations=" << performanceSnapshot.stopLevelDeallocations;
    return oss.str();
}
...
void Book::addLimit(int limitPrice, bool buyOrSell)
{
    ...
    performanceSnapshot.limitAllocations++;
...
void Book::addStop(int stopPrice, bool buyOrSell)
{
    ...
    performanceSnapshot.stopLevelAllocations++;
...
void Book::stopLimitOrderToLimitOrder(Order* headOrder, bool buyOrSell)
{
    ...
    if (shares != 0)
    {
        ...
        auto limitIt = limitMap.find(headOrder->getLimit());
        if (limitIt == limitMap.end())
        {
            addLimit(headOrder->getLimit(), buyOrSell);
            limitIt = limitMap.find(headOrder->getLimit());
        }
        limitIt->second->append(headOrder);
...
void Book::recordOperationStats()
{
    performanceSnapshot.executedOrders += executedOrdersCount;
    performanceSnapshot.avlTreeBalances += AVLTreeBalanceCount;
}
...
BookPerformanceSnapshot Book::getPerformanceSnapshot() const
{
    return performanceSnapshot;
}

void Book::resetPerformanceSnapshot()
{
    performanceSnapshot = {};
}
